...
if ($id) {
  ...
  if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
      $products[] = $row; // Thêm thông tin của sản phẩm vào mảng $products
    }
  }
  $db->close();
}
      ...
      function openModal() {
        ...
        xhttp.onreadystatechange = function() {
          if (this.readyState === 4 && this.status === 200) {
            ...
            console.log(response); // Ví dụ: In ra dữ liệu từ cơ sở dữ liệu
          }
        };

        xhttp.open("GET", "<?php echo $_SERVER['PHP_SELF']; ?>?id=" + id, true);
        xhttp.send();
        <?php
        ...
        modalText.textContent = <?php echo $value; ?>;
